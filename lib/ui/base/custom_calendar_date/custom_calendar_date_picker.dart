...
import 'custom_calendar_date_config.dart';
...
class CustomCalendarDatePicker extends StatefulWidget {
  CustomCalendarDatePicker({
    ...
    this.selectedDayChange,
    this.initialSelectDate,
    this.enableClickOnlyHoliday = true,
    Key? key,
  }) : super(key: key) {
    var ok = true;
    var notOk = false;
    var isInitialValueValid = holidayValue.length > 1
        ? (config.calendarType == CustomCalendarDatePickerType.range
            ? (holidayValue[0] == null
                ? (holidayValue[1] != null ? notOk : ok)
                : (holidayValue[1] != null
                    ? (holidayValue[1]!.isBefore(holidayValue[0]!) ? notOk : ok)
                    : ok))
            : ok)
        : ok;
    assert(
  ...
  final List<DateTime?> holidayValue;
  ...
  final ValueChanged<DateTime>? selectedDayChange;

  /// The calendar configurations
  final CustomCalendarDatePickerConfig config;

  final bool enableClickOnlyHoliday;
...
class _CustomCalendarDatePickerState extends State<CustomCalendarDatePicker> {
  ...
  late List<DateTime?> _holidayDates;
  ...
  void initState() {
    ...
    var initialDate = widget.holidayValue.isNotEmpty &&
            widget.holidayValue[0] != null
        ? DateTime(widget.holidayValue[0]!.year, widget.holidayValue[0]!.month)
        : DateUtils.dateOnly(DateTime.now());
    _mode = config.calendarViewMode;
    _currentDisplayedMonthDate = DateTime(initialDate.year, initialDate.month);
    _holidayDates = widget.holidayValue;
    _selectedDate = widget.initialSelectDate ?? DateTime.now();
    monthFormat = DateFormat('MMMM yyyy',
        widget.config.yearFormat == CustomYearFormat.th ? 'th' : 'en');
  }
  ...
  void didChangeDependencies() {
    ...
    if (!_announcedInitialDate && _holidayDates.isNotEmpty) {
      _announcedInitialDate = true;
      for (var date in _holidayDates) {
        if (date != null) {
  ...
  void _handleMonthChanged(DateTime date, {bool fromYearPicker = false}) {
    setState(() {
      ...
      if (fromYearPicker) {
        var holidayDatesInThisYear = widget.holidayValue
            .where((d) => d?.year == date.year)
            .toList()
          ..sort((d1, d2) => d1!.compareTo(d2!));
        if (holidayDatesInThisYear.isNotEmpty) {
          newDisplayedMonthDate =
              DateTime(date.year, holidayDatesInThisYear[0]!.month);
        }
      }
  ...
  void _handleDayChanged(DateTime value) {
    _vibrate();
    setState(() {
      List<DateTime?> holidayDates = [..._holidayDates];
      holidayDates.removeWhere((d) => d == null);

      if (widget.config.calendarType == CustomCalendarDatePickerType.single) {
        holidayDates = [value];
      } else if (widget.config.calendarType ==
          CustomCalendarDatePickerType.multi) {
        if (widget.enableClickOnlyHoliday) {
          var index =
              holidayDates.indexWhere((d) => DateUtils.isSameDay(d, value));
          if (index != -1) {
            _selectedDate = value;
            widget.selectedDayChange?.call(value);
          } else {}
        } else {
          _selectedDate = value;
          widget.selectedDayChange?.call(value);
          ...
          CustomCalendarDatePickerType.range) {
        if (holidayDates.isEmpty) {
          holidayDates.add(value);
        } else {
          var isRangeSet = holidayDates.length > 1 && holidayDates[1] != null;
          var isSelectedDayBeforeStartDate = value.isBefore(holidayDates[0]!);

          if (isRangeSet || isSelectedDayBeforeStartDate) {
            holidayDates = [value, null];
          } else {
            holidayDates = [holidayDates[0], value];
          }
        }
      }
      ...
      var isValueDifferent =
          widget.config.calendarType != CustomCalendarDatePickerType.single ||
              !DateUtils.isSameDay(holidayDates[0],
                  _holidayDates.isNotEmpty ? _holidayDates[0] : null);
      if (isValueDifferent) {
        _holidayDates = holidayDates.map((e) => e).toList();
        widget.onValueChanged?.call(_holidayDates);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    assert(debugCheckHasMaterialLocalizations(context));
    assert(debugCheckHasDirectionality(context));
    var displayTitle = widget.config.yearFormat == CustomYearFormat.th
        ? DateTime.utc(_currentDisplayedMonthDate.year + 543,
            _currentDisplayedMonthDate.month, _currentDisplayedMonthDate.day)
        : _currentDisplayedMonthDate;
    return Stack(
      children: <Widget>[
        SizedBox(
        ...
        _DatePickerModeToggleButton(
          config: widget.config,
          mode: _mode,
          title: monthFormat.format(displayTitle),
          onTitlePressed: () {
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void _handleGridFocusChange(bool focused) {
    setState(() {
      if (focused && _focusedDay == null && widget.holidayDates.isNotEmpty) {
        if (DateUtils.isSameMonth(widget.holidayDates[0], _currentMonth)) {
          _focusedDay = widget.holidayDates[0];
  ...
  Widget _buildItems(BuildContext context, int index) {
    ...
    return _DayPicker(
      ...
      holidayDates:
          (widget.holidayDates..removeWhere((d) => d == null)).cast<DateTime>(),
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final bool isHoliday =
            widget.holidayDates.any((d) => DateUtils.isSameDay(d, dayToBuild));
        ...
        if (isHoliday && !isSelectedDay) {
          // The selected day gets a circle background highlight, and a
          // contrasting text color.
          dayColor = selectedDayColor;
          decoration = BoxDecoration(
            borderRadius: widget.config.dayBorderRadius,
            color: widget.config.holidayHighLightColor ?? selectedDayBackground,
          ...
          dayColor =
              widget.config.selectedDayHighlightColor ?? selectedDayColor;
          decoration = BoxDecoration(
            borderRadius: widget.config.dayBorderRadius,
        ...
        if (isHoliday && !isSelectedDay) {
          customDayTextStyle = widget.config.holidayTextStyle;
        } else if (isSelectedDay) {
        ...
        if (widget.config.calendarType == CustomCalendarDatePickerType.range) {
          if (widget.holidayDates.length == 2) {
            var startDate = DateUtils.dateOnly(widget.holidayDates[0]);
            var endDate = DateUtils.dateOnly(widget.holidayDates[1]);
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    if (isSelected) {
      ...
      textColor = widget.config.holidayHighLightColor ?? colorScheme.primary;
    } else {
      textColor = colorScheme.onSurface.withOpacity(0.87);
    }
    final TextStyle? itemStyle =
        widget.config.yearTextStyle?.apply(color: textColor) ??
            textTheme.bodyText1?.apply(color: textColor);

    BoxDecoration? decoration;
    if (isSelected) {
      decoration = BoxDecoration(
        color: widget.config.holidayHighLightColor ?? colorScheme.primary,
    ...
    // Note: for display year พศ.
    String yearDisplay = widget.config.yearFormat == CustomYearFormat.th
        ? (year + 543).toString()
        : year.toString();
