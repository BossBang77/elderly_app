...
class FoodLogBloc extends Bloc<FoodLogEvent, FoodLogState> {
  FoodLogBloc(
      MealType mealType, MealRecordRepositoryProtocol mealRecordRepository)
      : _mealRecordRepository = mealRecordRepository,
        _mealType = mealType,
        super(FoodLogState(
            foods: mealRecordRepository.getCurrentMealListFrom(mealType),
            mealType: mealType)) {
    ...
    on<ResetStateOfSaved>(
      (event, emit) {
        emit(state.copyWith(isSavedCompleted: IsSaveState.initial));
      },
    );

    MealType.values.forEach((element) {
      mealRecordRepository.mealRecordStreamFor(element).listen((event) => {
            add(FoodLogListUpdated(
                mealType: MealType.breakfast, mealList: event))
          });
    });
  }
  ...
  void _onAddButtonTapped(
      FoodLogAddMoreFoodButtonTapped event, Emitter<FoodLogState> emit) {}

  void _onSaveButtonTapped(
      FoodLogSaveButtonTapped event, Emitter<FoodLogState> emit) async {
    ...
    response.fold((error) {
      emit(state.copyWith(isSavedCompleted: IsSaveState.fail));
    }, (response) {
      MealType.values.forEach((mealType) {
        final snapshot =
            _mealRecordRepository.mealRecordStreamFor(mealType).value;
        _mealRecordRepository
            .mealRecordStreamFor(mealType)
            .createSnapshot(snapshot);
      });
      emit(state.copyWith(isSavedCompleted: IsSaveState.success));
    });
  }

  void _onFoodSelected(FoodLogFoodSelected event, Emitter<FoodLogState> emit) {}

  void _onFoodLogListUpdated(
      FoodLogListUpdated event, Emitter<FoodLogState> emit) {
    emit(state.copyWith(foods: event.mealList));
  }

  void _onBackButtonTapped(
      FoodLogBackButtonTapped event, Emitter<FoodLogState> emit) {
    _mealRecordRepository.mealRecordStreamFor(_mealType).restoreState();
  }

  void _onFoodLogMealItemRemoved(
      FoodLogMealItemRemoved event, Emitter<FoodLogState> emit) {
