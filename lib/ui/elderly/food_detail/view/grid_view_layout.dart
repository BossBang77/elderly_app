...
class GridViewLayout {
  GridViewLayout({
    ...
    gridItemWidth = (gridViewWidth / numberOfItemsPerRow) - ((numberOfItemsPerRow - 1) * spacing);
    numberOfRow = (((dataSource.length - 1 ) / numberOfItemsPerRow).floor() + 1).toInt();
  ...
  List<double> _textHeight() {
    return List<int>.generate(numberOfRow, (i) => i)
      .map((rowNumber) {
        final int itemInRow = rowNumber == numberOfRow - 1 ? numberOfItemsPerRow - (((rowNumber + 1) * numberOfItemsPerRow) - dataSource.length) : numberOfItemsPerRow;
        List<double> itemsInRowHeight = List<int>.generate(itemInRow, (i) => i + rowNumber).map((index) {
          final titleTextViewHeight = _textViewHeightFromText(dataSource[index].name);
          final valueTextViewHeight = _textViewHeightFromText(dataSource[index].unit.toString() + ' ' + dataSource[index].unitName);

          final textViewHeight = titleTextViewHeight + valueTextViewHeight;
          return textViewHeight;
        }).toList();
        return itemsInRowHeight.reduce((value, element) => value > element ? value : element);
      }).toList();
  }
  ...
  double get gridViewHeight => List<int>.generate(numberOfRow, (i) => i).map((index) => heightForRowAt(index)).reduce((value, element) => value + element) + 16;
}