...
class FoodFilterState extends Equatable {
  ...
  final List<SubMenuModel> selectedTypes;
  ...
  FoodFilterState copyWith(
      ...
      List<SubMenuModel>? selectedTypes,
    ...
    return FoodFilterState(
        isCongentialDeceaseSelected:
            isCongentialDeceaseSelected ?? this.isCongentialDeceaseSelected,
        isFoodAllergiesSelected:
            isFoodAllergiesSelected ?? this.isFoodAllergiesSelected,
  ...
  Map<String, dynamic> getMappedFilter() {
    ...
    if (isCongentialDeceaseSelected) {
      ...
      mappedFilter['selectedDisease'] =
          selectedDisease.map((disease) => disease.name).toList();
    }
    if (isFoodAllergiesSelected) {
      mappedFilter['isFoodAllergiesSelected'] = isFoodAllergiesSelected;
      mappedFilter['selectedFoodAllergy'] = selectedFoodAllergy;
    }
    if (selectedTypes.isNotEmpty) {
      mappedFilter['foodTypes'] = selectedTypes;
    }
    if (minimumCalories.isNotEmpty) {
      mappedFilter['minimumCalories'] = int.tryParse(minimumCalories) ?? 0;
    }
    if (maximumCalories.isNotEmpty) {
      mappedFilter['maximumCalories'] = int.tryParse(maximumCalories) ?? 0;
    }
